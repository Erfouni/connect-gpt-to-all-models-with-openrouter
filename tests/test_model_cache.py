...
server.py runs on ThreadingHTTPServer, so /models, /run and /compare are served
concurrently. Every one of them reads the cached catalog, and GET /models sorts
what it reads. CPython empties a list for the whole duration of an in-place
sort, including while the key function runs, so a caller that sorts the cached
list itself blanks the catalog for every other thread that is reading it.
"""
