#!/usr/bin/env python3
"""Small OpenRouter gateway used by the MCP model router.

The gateway binds to loopback by default, reads credentials from the process
environment or a local .env file, and never returns the API key.
"""
...
def load_env_file(path: Path) -> None:
    """Load a minimal KEY=VALUE .env file without overriding real env vars."""
    if not path.exists():
        return
...
AGENT_TOKEN = os.environ.get("OPENROUTER_AGENT_TOKEN", "").strip()
...
def api_key() -> str:
    ...
    if not key:
        raise GatewayError("OPENROUTER_API_KEY is not configured", 503)
    return key
...
def openrouter_request(
    ...
    headers = {
        ...
        "HTTP-Referer": os.environ.get("OPENROUTER_SITE_URL", "https://localhost.invalid"),
        "X-OpenRouter-Title": os.environ.get(
            "OPENROUTER_APP_NAME", "ChatGPT Model Orchestrator"
        ),
    }
    ...
    try:
        ...
        try:
            upstream: Any = json.loads(raw)
        except json.JSONDecodeError:
            upstream = raw[:2000]
        raise GatewayError(
            "OpenRouter request failed",
            status=502,
            details={"upstream_status": exc.code, "upstream": upstream},
        ) from exc
    except urllib.error.URLError as exc:
        raise GatewayError(
            "Could not reach OpenRouter", status=502, details=str(exc.reason)
        ) from exc
...
def newest_matching(
    ...
    for item in get_models():
        ...
        if not model_id.startswith(lowered_prefixes):
            continue
...
def configured_or_newest(
    ...
    return configured or newest_matching(prefixes, required_terms, excluded_terms)
...
def resolve_model(requested: str) -> str:
    ...
    if normalized in {"glm", "glm-5", "glm5", "جی‌ال‌ام", "جی ال ام"}:
        return configured_or_newest(
            "DEFAULT_GLM_MODEL",
            ("z-ai/glm", "thudm/glm"),
            excluded_terms=("image", "vision"),
        )
    ...
    if normalized in {"kimi", "کیمی"}:
        return configured_or_newest(
            "DEFAULT_KIMI_MODEL", ("moonshotai/kimi", "moonshot/kimi")
        )
    if normalized in {"claude", "کلاد"}:
        return configured_or_newest(
            "DEFAULT_CLAUDE_MODEL", "anthropic/claude", required_terms=("sonnet",)
        )
    if normalized in {"deepseek", "دیپ‌سیک", "دیپ سیک"}:
        return configured_or_newest("DEFAULT_DEEPSEEK_MODEL", "deepseek/")
    if normalized in {"qwen", "کوئن"}:
        return configured_or_newest("DEFAULT_QWEN_MODEL", "qwen/")
...
def build_messages(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(prompt) > MAX_PROMPT_CHARS:
        raise GatewayError(
            "Prompt is too large", status=413, details={"max_prompt_chars": MAX_PROMPT_CHARS}
        )
...
def compare_models(data: dict[str, Any]) -> dict[str, Any]:
    ...
    with ThreadPoolExecutor(max_workers=len(requested_models)) as executor:
        ...
        for future in as_completed(futures):
            ...
            except Exception as exc:  # Return per-model errors without aborting peers.
                if isinstance(exc, GatewayError):
                    results.append(
                        {"model_requested": model, "error": str(exc), "details": exc.details}
                    )
                else:
...
class Handler(BaseHTTPRequestHandler):
    server_version = "OpenRouterAgent/1.0"
    ...
    def authorized(self) -> bool:
        ...
        return hmac.compare_digest(supplied, AGENT_TOKEN)
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if parsed.path == "/models":
                ...
                self.send_json(
                    200, {"models": [public_model(item) for item in models[:limit]]}
                )
                return
...
def main() -> None:
    ...
    if HOST not in loopback_names and not AGENT_TOKEN:
        raise SystemExit(
            "Refusing a non-loopback bind without OPENROUTER_AGENT_TOKEN. "
            "Set a strong token or bind to 127.0.0.1."
        )
    httpd = ThreadingHTTPServer((HOST, PORT), Handler)
    print(
        json.dumps({"service": "openrouter-agent", "url": f"http://{HOST}:{PORT}"}),
        flush=True,
    )
    httpd.serve_forever()
